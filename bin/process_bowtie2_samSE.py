...
def process_unpaired_sam(sam_path, out_path, genomeid_taxid_map):
    """Process unpaired SAM file into a TSV."""
    with open_by_suffix(sam_path) as inf, open_by_suffix(out_path, "w") as outf:
        write_sam_headers_unpaired(outf)  # Write headers (we'll define this function later)
        
        while True:
            line = get_next_alignment(inf)
            if line is None:
                break
            
            read_dict = process_sam_alignment(line, genomeid_taxid_map, False)
            
            if read_dict["pair_status"] != "UU":
                raise ValueError(f"Invalid pair status for unpaired read: {read_dict['query_name']}, {read_dict['pair_status']}")
            
            output_line = process_sam_unpaired_read(read_dict)
            outf.write(output_line)

def write_sam_headers_unpaired(out_file):
    """Write header line to new TSV for unpaired reads."""
    headers = ["query_name", "genome_id", "taxid", "alignment_score", "next_best_alignment", "edit_distance", 
               "ref_start", "map_qual", "cigar", "query_len", "query_seq", "pair_status"]
    header_line = join_line(headers)
    out_file.write(header_line)

def process_sam_unpaired_read(read_dict):
    """Process an unpaired alignment from a SAM file."""
    return get_line_unpaired(read_dict["query_name"], read_dict["genome_id"], read_dict["taxid"],
                             read_dict["alignment_score"], read_dict["next_best_alignment"],
                             read_dict["edit_distance"], read_dict["ref_start"], read_dict["map_qual"],
                             read_dict["cigar"], read_dict["query_len"], read_dict["query_seq"],
                             read_dict["pair_status"])

def get_line_unpaired(query_name, genome_id, taxid, alignment_score, next_alignment_score, 
                      edit_distance, ref_start, map_qual, cigar, query_len, query_seq, pair_status):
    """Convert a list of arguments into an output line for unpaired reads."""
    fields = [query_name, genome_id, taxid, alignment_score, next_alignment_score, edit_distance,
              ref_start, map_qual, cigar, query_len, query_seq, pair_status]
